...
PyTimer::PyTimer(py::module_& m)
{
    ...
    class_Timer.def_static("enable_all",
                           &Timer::enable_all,
                           R"(Enable all timers for performance measurement.)");
    class_Timer.def_static("disable_all", &Timer::disable_all, R"(Disable all timers.)");
    class_Timer.def_static("report", []() { Timer::report(); }, R"(Print timing report to the console.)");
    class_Timer.def_static("report_as_json",
