...
PyFactory::PyFactory(py::module_& m)
{
    m.def(
        "tetmesh",
        [](numpy_array<Float> Vs, numpy_array<IndexT> Ts)
...
Returns:
    ...
    m.def(
        "trimesh",
        [](numpy_array<Float> Vs, numpy_array<IndexT> Fs) -> SimplicialComplex
        {
...
Returns:
    ...
    m.def(
        "linemesh",
        [](numpy_array<Float> Vs, numpy_array<IndexT> Es)
...
Returns:
    ...
    m.def(
        "pointcloud",
        [](numpy_array<Float> Vs) { return pointcloud(as_span_of<Vector3>(Vs)); },
        py::arg("Vs"),
...
Returns:
    ...
    m.def(
        "ground",
        [](Float height, numpy_array<Float> N)
...
Returns:
    ...
    m.def(
        "halfplane",
        [](numpy_array<Float> P, numpy_array<Float> N)
