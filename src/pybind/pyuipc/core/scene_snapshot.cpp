...
PySceneSnapshot::PySceneSnapshot(py::module_& m)
{
    ...
    class_SceneSnapshot.def(py::init<const Scene&>(),
                            py::arg("scene"),
                            R"(Create a snapshot from a scene.
Args:
    scene: Scene to snapshot.)");

    auto class_SceneSnapshotCommit = py::class_<SceneSnapshotCommit>(
        m, "SceneSnapshotCommit", R"(SceneSnapshotCommit class representing changes between two scene snapshots.)");
