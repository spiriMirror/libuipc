...
PySceneFactory::PySceneFactory(py::module& m)
{
    auto class_SceneFactory =
        py::class_<SceneFactory>(m, "SceneFactory", R"(SceneFactory class for creating scenes from various sources.)")
            .def(py::init<>(), R"(Create a new SceneFactory instance.)")
            .def("from_json",
                 &SceneFactory::from_json,
                 py::arg("json"),
...
Returns:
    Scene: Created scene.)")
            .def("to_json",
                 &SceneFactory::to_json,
                 R"(Convert factory state to JSON.
Returns:
    dict: JSON representation of the factory state.)");

    class_SceneFactory.def("from_snapshot",
                           &SceneFactory::from_snapshot,
                           py::arg("snapshot"),
...
Returns:
    Scene: Created scene.)");
    class_SceneFactory.def("commit_from_json",
                           &SceneFactory::commit_from_json,
                           py::arg("json"),
                           R"(Apply commit changes from JSON.
Args:
    json: JSON dictionary containing commit changes.)");
    class_SceneFactory.def("commit_to_json",
                           &SceneFactory::commit_to_json,
                           py::arg("reference"),
