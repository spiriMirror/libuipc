 ...
 * The retained ParMETIS/DGL support algorithms remain source-equivalent.
 */
...
void FM_2WayNodeRefine1SidedP(ctrl_t* ctrl, graph_t* graph, idx_t* hmarker, real_t ubfactor, idx_t npasses)
{
        ...
        for(nswaps = 0; nswaps < nvtxs; nswaps++)
        {
         ...
         of over-running the mind array.  */
            ...
            newdiff = abs(pwgts[to] + vwgt[higain]
                          - (pwgts[from] - rinfo[higain].edegrees[from]));
    ...
    *****************************************************************/
        ...
        IFSET(ctrl->dbglvl,
              METIS_DBG_REFINE,
              printf("\tMinimum sep: %6" PRIDX " at %5" PRIDX ", PWGTS: [%6" PRIDX
                     " %6" PRIDX "], NBND: %6" PRIDX ", QSIZE: %6" PRIDX "\n",
                     mincut,
                     mincutorder,
                     pwgts[0],
                     pwgts[1],
...
void FM_2WayNodeRefine2SidedP(ctrl_t* ctrl, graph_t* graph, idx_t* hmarker, real_t ubfactor, idx_t npasses)
{
    ...
    for(pass = 0; pass < npasses; pass++)
    {
        ...
        for(ii = 0; ii < nbnd; ii++)
        {
            ...
            {
                rpqInsert(queues[hmarker[i]],
                          i,
                          vwgt[i] - rinfo[i].edegrees[(hmarker[i] + 1) % 2]);
            ...
            for(j = xadj[higain]; j < xadj[higain + 1]; j++)
            {
                ...
                { /* This vertex is pulled into the separator */
                    ...
                    if(moved[k] == -1)
                    {
                        ...
                        {
                            rpqInsert(queues[hmarker[k]],
                                      k,
                                      vwgt[k] - edegrees[(hmarker[k] + 1) % 2]);
