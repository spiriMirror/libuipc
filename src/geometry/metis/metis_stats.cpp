 ...
 * The partition-statistics algorithms remain source-equivalent.
 */
...
void ComputePartitionInfoBipartite(graph_t* graph, idx_t nparts, idx_t* where)
{
    ...
    if(ncon == 1)
    {
        printf("\tBalance: %5.3" PRREAL " out of %5.3" PRREAL "\n",
               1.0 * nparts * kpwgts[iargmax(nparts, kpwgts, 1)]
                   / (1.0 * isum(nparts, kpwgts, 1)),
               1.0 * nparts * vwgt[iargmax(nvtxs, vwgt, 1)]
                   / (1.0 * isum(nparts, kpwgts, 1)));
    }
    else
    {
    ...
    printf("Min/Max/Avg/Bal # of adjacent     subdomains: %5" PRIDX " %5" PRIDX
           " %5" PRIDX " %7.3" PRREAL "\n",
           kpwgts[iargmin(nparts, kpwgts, 1)],
           kpwgts[iargmax(nparts, kpwgts, 1)],
    ...
    printf("Min/Max/Avg/Bal # of adjacent subdomain cuts: %5" PRIDX " %5" PRIDX
           " %5" PRIDX " %7.3" PRREAL "\n",
           kpwgts[iargmin(nparts, kpwgts, 1)],
           kpwgts[iargmax(nparts, kpwgts, 1)],
           isum(nparts, kpwgts, 1) / nparts,
    ...
    printf("Min/Max/Avg/Bal/Frac # of interface    nodes: %5" PRIDX " %5" PRIDX
           " %5" PRIDX " %7.3" PRREAL " %7.3" PRREAL "\n",
           kpwgts[iargmin(nparts, kpwgts, 1)],
           kpwgts[iargmax(nparts, kpwgts, 1)],
           isum(nparts, kpwgts, 1) / nparts,
           1.0 * nparts * kpwgts[iargmax(nparts, kpwgts, 1)]
...
void ComputePartitionBalance(graph_t* graph, idx_t nparts, idx_t* where, real_t* ubvec)
{
    ...
    else
    {
        for(j = 0; j < ncon; j++)
        {
            ...
            ubvec[j] = 1.0 * nparts * kpwgts[iargmax(nparts, kpwgts, 1)]
                       / (1.0 * isum(nparts, kpwgts, 1));
        }
    }
...
real_t ComputeElementBalance(idx_t ne, idx_t nparts, idx_t* where)
{
    ...
    balance = 1.0 * nparts * kpwgts[iargmax(nparts, kpwgts, 1)]
              / (1.0 * isum(nparts, kpwgts, 1));
