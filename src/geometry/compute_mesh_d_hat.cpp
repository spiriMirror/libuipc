...
S<AttributeSlot<Float>> compute_mesh_d_hat(SimplicialComplex& R, Float max_d_hat)
{
    if(R.dim() == 0)
    {
        log::warn("Can't compute proper d_hat on a point cloud, skip.");
        return nullptr;
    }

    if(R.edges().size() == 0)
    {
        log::warn("Can't compute proper d_hat on a mesh without edges, skip.");
        return nullptr;
    }
    ...
    if(!edge_is_surf)
    {
        log::warn("Can't compute proper d_hat on a mesh without `is_surf` attribute, did you forget to call `label_surface()`?");
        return nullptr;
    }
    ...
    if(!any_surf)
    {
        log::warn("Can't compute proper d_hat on a mesh without surface edge, skip.");
        return nullptr;
    }
