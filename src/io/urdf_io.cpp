...
class UrdfIO::Impl
{
  public:
    ...
    UrdfController read(uipc::core::Object& object, std::string_view urdf_path)
    {
        ...
        log::info(URDFIO_INFO "Model name: {}", model->name_);
        log::info(URDFIO_INFO "Number of links: {}", links.size());
        log::info(URDFIO_INFO "Number of joints: {}", joints.size());
        log::info(URDFIO_INFO "Processing links and joints...");
        ...
        {
            // Process each link
            log::info(URDFIO_INFO "- Link: {}", name);
            if(!check_link(link))
                continue;
    ...
    bool check_link(const urdf::LinkSharedPtr& link)
    {
        ...
        {
            log::info(URDFIO_INFO "> Link has no collision geometry, skip.");
            return false;
        }

        if(link->collision->geometry == nullptr)
        {
            log::info(URDFIO_INFO "> Link has no geometry, skip.");
            return false;
        }
        ...
        {
            log::info(URDFIO_INFO "> Link type {} is not supported yet, skip.",
        ...
        if(mesh == nullptr)
        {
            log::info(URDFIO_INFO "> Link mesh is null, skip.");
            return false;
        }

        if(mesh->filename.empty())
        {
            log::info(URDFIO_INFO "> Link mesh filename is empty, skip.");
            return false;
        }
                                               ...
                                               const urdf::Mesh&          mesh)
    {
        ...
        log::info(URDFIO_INFO "Loading mesh file: {}", f);
    ...
    void build_joint_infos()
    {
        ...
        for(auto&& [name, joint] : model->joints_)
        {
            if(joint->type == urdf::Joint::REVOLUTE)
            {
                ...
                log::info("Revolute Joint: {}", joint->name);
            ...
            else if(joint->type == urdf::Joint::FIXED)
            {
                ...
                log::info("Fixed Joint: {}", joint->name);
            }
            else
            {
                UIPC_ASSERT(false,
