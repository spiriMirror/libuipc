...
NoneSimEngine::NoneSimEngine(EngineCreateInfo* info)
    : SimEngine(info)
{
    log::info("[NoneEngine] Constructor called.");
    log::info(R"(Hello, this is the NoneEngine from libuipc backends.
...
{
    log::info("[NoneEngine] do_init() called.");
...
{
    m_frame++;
    log::info("[NoneEngine] do_advance() called.");
}

void NoneSimEngine::do_sync()
{
    log::info("[NoneEngine] do_sync() called.");
}

void NoneSimEngine::do_retrieve()
{
    log::info("[NoneEngine] do_receive() called.");
}

NoneSimEngine::~NoneSimEngine()
{
    log::info("[NoneEngine] Destructor called.");
}
