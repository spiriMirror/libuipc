    ...
    auto filter = [&](DeviceBuffer<int>& cnt) {
        ParallelFor()
            ...
            .apply(cnt.size(), [
                ...
                large_cnt = static_cast<int>(floor(log(1e-20) / log(decay_factor)))
            ] __device__ (int i) mutable {
                if (cnt(i) >= 1) cnt(i) = large_cnt;
            });
    };
    ...
    auto merge = [&](DeviceBuffer<Vector2i>& idx,
        ...
        ParallelFor()
            ...
            .apply(N, [
                ...
                threshold = static_cast<int>(floor(log(0.01) / log(decay_factor)))
            ] __device__ (int i) mutable {
        ...
        ParallelFor()
            ...
            .apply(N, [
                ...
                sort_idx = sort_index.viewer().name("sort_idx")
            ] __device__ (int i) mutable {
        ...
        ParallelFor()
            ...
            .apply(N, [
                ...
                total_count = total_count.viewer().name("total_count")
            ] __device__ (int i) mutable {
                if (flag(i)) {
                    auto idx = sort_idx(i);
                    auto j = offset(i);
...
void GlobalActiveSetManager::Impl::linearize_constraints() {
    ...
    ParallelFor().file_line(__FILE__, __LINE__).apply(PT_idx.size(), [
        ...
        d_grad = PT_d_grad.viewer().name("d_grad")
    ...
    ParallelFor().file_line(__FILE__, __LINE__).apply(EE_idx.size(), [
        thicknesses = thicknesses.cviewer().name("thicknesses"),
        d_hats = d_hats.cviewer().name("d_hats"),
...
void GlobalActiveSetManager::Impl::update_slack() {
    ...
    ParallelFor().file_line(__FILE__, __LINE__).apply(PTs.size(), [
        ...
        PT_lambda = PT_lambda.cviewer().name("PT_lambda"),
        d0 = PT_d0.viewer().name("d0"),
        slack = PT_slack.viewer().name("slack")
    ] __device__ (int idx) mutable {
        auto PT = PTs(idx);
    ...
    ParallelFor().file_line(__FILE__, __LINE__).apply(EEs.size(), [
        mu = mu,
        ...
        EE_lambda = EE_lambda.cviewer().name("EE_lambda"),
        d0 = EE_d0.viewer().name("d0"),
        slack = EE_slack.viewer().name("slack")
    ] __device__ (int idx) mutable {
        auto EE = EEs(idx);
...
void GlobalActiveSetManager::Impl::update_lambda() {
    ...
    ParallelFor().file_line(__FILE__, __LINE__).apply(PTs.size(), [
        ...
        PT_cnt = PT_cnt.viewer().name("PT_cnt")
    ] __device__ (int idx) mutable {
        auto PT = PTs(idx);
        auto d_grad = PT_d_grad(idx);
        auto d = d0(idx), &lambda = PT_lambda(idx), d_shift = 0.0;
    ...
    ParallelFor().file_line(__FILE__, __LINE__).apply(EEs.size(), [
        ...
        EE_cnt = EE_cnt.viewer().name("EE_cnt")
    ] __device__ (int idx) mutable {
        auto EE = EEs(idx);
        auto d_grad = EE_d_grad(idx);
        auto d = d0(idx), &lambda = EE_lambda(idx), d_shift = 0.0;
...
GlobalActiveSetManager::NonPenetratePositionsInfo::NonPenetratePositionsInfo(Impl *impl, SizeT offset, SizeT count) noexcept
    : m_impl(impl), m_offset(offset), m_count(count) {
}
