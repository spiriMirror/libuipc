...
namespace detail
{
    ...
    UIPC_GENERIC void eval_penalty_impl(
        const T& x, const T& lower, const T& upper, const T& strength, T& E, T& dE_dx, T& d2E_dx2)
    {
...
template <typename T>
UIPC_GENERIC void eval_penalty(
    const T& x, const T& lower, const T& upper, const T& strength, T& E, T& dE_dx, T& d2E_dx2)
{
    detail::eval_penalty_impl<true, true>(x, lower, upper, strength, E, dE_dx, d2E_dx2);
}

template <typename T>
UIPC_GENERIC T eval_penalty_energy(const T& x, const T& lower, const T& upper, const T& strength)
{
    T E       = static_cast<T>(0);
    T dE_dx   = static_cast<T>(0);
    T d2E_dx2 = static_cast<T>(0);
    detail::eval_penalty_impl<true, false>(x, lower, upper, strength, E, dE_dx, d2E_dx2);
    return E;
}

template <typename T>
UIPC_GENERIC void eval_penalty_derivatives(
    const T& x, const T& lower, const T& upper, const T& strength, T& dE_dx, T& d2E_dx2)
{
    T E = static_cast<T>(0);
    detail::eval_penalty_impl<false, true>(x, lower, upper, strength, E, dE_dx, d2E_dx2);
}
}  // namespace uipc::backend::cuda::joint_limit
