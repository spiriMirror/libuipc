...
namespace uipc::backend::cuda {
    ...
    void ALVertexHalfPlaneNormalContact::Impl::do_compute_energy(GlobalContactManager::EnergyInfo &info) {
        ...
        ParallelFor().file_line(__FILE__, __LINE__).apply(PH_size, [
            ...
            x = x.cviewer().name("x"),
            Es = PH_energies.viewer().name("Es")
        ] __device__ (int idx) mutable {
            auto vI = PHs(idx);
            Es(idx) = half_plane_penalty_energy(pow(decay, cnt(idx)) * mu, d0(idx), d_grad(idx), x(vI));
        });
    }
    ...
    void ALVertexHalfPlaneNormalContact::Impl::do_assemble(GlobalContactManager::GradientHessianInfo &info) {
        ...
        ParallelFor().file_line(__FILE__, __LINE__).apply(PH_size, [
            ...
            Hs = PH_hess.viewer().name("Hs")
        ] __device__ (int idx) mutable {
            auto vI = PHs(idx);
            Vector3 G;
            Matrix3x3 H;
            half_plane_penalty_gradient_hessian(pow(decay, cnt(idx)) * mu, d0(idx), d_grad(idx), x(vI), G, H);
