...
Json Engine::default_config()
{
    ...
    if(config_path)
    {
        ...
        if(!ifs)
        {
            log::warn("Load default config file [{}] failed, fallback to default config.",
                         config_path);
        }
        else
        {
            try
            {
                ...
                log::info("Override default config using config file [{}] successfully.",
                             config_path);
            }
            catch(const std::exception& e)
            {
                log::warn("Load default config file [{}] failed: {}, rollback to default config.",
                             config_path,
                             e.what());
            }
        }
