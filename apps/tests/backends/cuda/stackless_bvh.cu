...
std::vector<Vector2i> culbvh_cp(span<const AABB> aabbs)
{
    ::culbvh::LBVHStackless bvh;
    bvh.type = 1;
    ...
    DeviceBuffer<::culbvh::aabb> d_aabbs = cul_aabbs;


    bvh.compute(d_aabbs.data(), d_aabbs.size());
    auto num = bvh.query();

    std::cout << "num collision pairs: " << num << std::endl;
    return {};
}
