...
- The UID tables in `docs/specification/constitution_uid.md` and `implicit_geometry_uid.md` are generated by `scripts/gen_uid_doc.py`, but the constitution parser currently misses statement-assigned `UIDInfo` registrations even when `--check` passes (doc 04/09). Do not treat that page as exhaustive.
