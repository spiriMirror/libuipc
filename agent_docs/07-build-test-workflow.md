...
- CMake ≥ 3.26, Python ≥ 3.11, CUDA ≥ 12.4 (12.8 for wheels), Vcpkg (set `CMAKE_TOOLCHAIN_FILE`).
- The vcpkg manifest is generated by `scripts/gen_vcpkg_json.py`. Base dependencies: eigen3, catch2, libigl, spdlog, fmt (pinned at 10.2.1), cppitertools, dylib, nlohmann-json, magic-enum, tinygltf, tbb, urdfdom, cpptrace, octree (private registry spiriMirror/vcpkg; the same registry also hosts ftetwild); GUI additionally pulls imgui/glfw3/opengl/freeglut/bgfx; VDB additionally pulls openvdb.
...
**Source glob**: all `file(GLOB...)` calls carry `CONFIGURE_DEPENDS`, so after adding or removing source files there is **no need to manually re-configure** — just build directly (CMake ≥3.12 VerifyGlobs mechanism).
