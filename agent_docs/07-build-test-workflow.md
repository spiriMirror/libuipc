...
- muda (CUDA wrapper library: kernel launch, buffer, linear algebra) is vendored at `src/backends/cuda/cuda_tool/muda/` and compiled together with the main repository sources; under `external/` only the in-tree GKlib/METIS sources remain.
...
- No C-style code or raw-pointer parameters (use `span<T>` / `muda::BufferView`); no `const std::string&` parameters (use `std::string_view`); no multiple inheritance.
...
**Simulation development** (simulation-dev): correctness > debuggability > performance; validate indices/shapes at boundaries; index guards at the top of kernels; `isfinite` at key checkpoints; `muda::debug_sync_all()` during debugging; debug loop: minimal reproduction → hypothesis verification → minimal fix → verify on the original scene → regression test.
